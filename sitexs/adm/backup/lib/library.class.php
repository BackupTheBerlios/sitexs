...
class library {
	...
	function defaultAction () {
		...
		$pagination=new pagination ("?chid=".$this->chid."&", $this->page, $adminConfig["recPerPage"], '', "library", "id");
		
		$res=$db->query("select id, name, short_text, time from library order by id desc ".$pagination->limit());
		$page =($this->page) ? "&page=".$this->page : "";
		while($data=$db->fetch_array($res)) {
			$res1=$db->query("select id, firstname, secondname, lastname from library_authors left join authors on library_authors.author = authors.id where library_authors.library=".$data["id"]." order by lastname, firstname, secondname");
	...
	function librarySelectI () {
		...
		$res=$db->query("select id, name, short_text, time from library order by id desc limit ".(($this->page-1)*$adminConfig["recPerPage"]).", ".$adminConfig["recPerPage"]);
		...
		while ($data=$db->fetch_array($res)) {
			...
			$checked=($data["id"]==$this->id || (!$this->id && $i==1)) ? " checked" : "";
