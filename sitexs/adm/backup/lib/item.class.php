...
class item {
	...
	function add () {
		...
		while($data=$db->fetch_array($res)) {
			$i++;
			$types.="<option".(($this->level==$i) ? " selected" : "")." value=\"$data[id]\">$data[title]</option>";
		}
	...
	function edit () {
		...
		$library=$db->fetch_array($res2);
		$library["chid"]=admin::getTypeID("library");
		$res1=$db->query("select id, firstname, secondname, lastname from library_authors left join authors on library_authors.author = authors.id where library_authors.library='".$library["id"]."' order by lastname, firstname, secondname");
		$typeAuthorsID=admin::getTypeID("authors");
		while ($data1=$db->fetch_array($res1)) {
			$library["fio"].="<li><a href=\"?chid=$typeAuthorsID&action=edit&id=$data1[id]\">".$data1["lastname"].(($data1["firstname"]) ? " ".$data1["firstname"] : "").(($data1["secondname"]) ? " ".$data1["secondname"] : "")."</a></li>";
		}
		$library["date"]=($library["time"]) ? @date("d.m.Y", $library["time"]) : "";
		$library["fio"]=($library["fio"]) ? "<ul>".$library["fio"]."</ul>" : "";
